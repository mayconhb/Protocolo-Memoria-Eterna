...
const VideoSession: React.FC<{ block: GuideBlock }> = ({ block }) => {
  ...
  return (
    <div className="flex flex-col items-center mb-8 animate-fade-in">
      ...
      <div className="relative w-64 h-64 sm:w-72 sm:h-72 mb-8 group">
        <div className={`absolute inset-0 rounded-full bg-indigo-500/30 blur-2xl transition-all duration-1000 ${isPlaying ? 'animate-[pulse_2s_cubic-bezier(0.4,0,0.6,1)_infinite] opacity-60 scale-105' : 'opacity-0 scale-100'}`}></div>
        <div className={`absolute inset-0 rounded-full bg-purple-500/20 blur-3xl transition-all duration-1000 ${isPlaying ? 'animate-[pulse_3s_cubic-bezier(0.4,0,0.6,1)_infinite] opacity-40 scale-110' : 'opacity-0 scale-100'}`}></div>
        ...
        {isPlaying && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 pointer-events-none">
            <div className="bg-black/40 backdrop-blur-md px-4 py-2 rounded-full text-white font-mono text-xl tracking-wider border border-white/20">
      ...
      >
        <div className="bg-white/20 p-1.5 rounded-full">
          {isPlaying ? (
            <div className="w-[22px] h-[22px] flex items-center justify-center gap-1">
...
const GuideBlockRenderer: React.FC<{ block: GuideBlock }> = ({ block }) => {
  ...
  switch (block.type) {
    ...
    case 'video_session':
      return <VideoSession block={block} />;
